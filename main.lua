...
local function isTargetName(name)
	...
	if TargetBrainrots[name] then
		return true
	end

	local key = normKey(name)

	for target in pairs(TargetBrainrots) do
		if normKey(target) == key then
			return true
		end
	end
...
local function getRemote(name, className)
	...
	if not remote then
		warn("[K2] Remote missing:", name)
		return nil
	end

	if className and not remote:IsA(className) then
		warn(
			"[K2] Wrong class for",
			name,
			"| expected:",
...
local READY_GUID =
	"23f15b0b-b633-4f6b-888f-5924b7425522"

local ACCEPT_GUID =
	"86eea964-f19e-4ac6-b401-a71ecc89e596"

local ACCEPT_INVITE_GUID =
	"8c94acca-6417-45e5-89f0-efb8b910cde7"

local CANCEL_GUID =
	"171b5ced-5729-49c0-8d80-9c1897ff1ea3"

local readyRE = getRemote(
	"RE/TradeService/Ready",
	"RemoteEvent"
)

local acceptRE = getRemote(
	"RE/TradeService/Accept",
	"RemoteEvent"
)

local acceptInviteRF = getRemote(
	"RF/TradeService/AcceptInvite",
	"RemoteFunction"
)

local createInviteRE = getRemote(
	"RE/TradeService/CreateInvite",
	"RemoteEvent"
)

local cancelRE = getRemote(
	"RE/TradeService/Cancel",
	"RemoteEvent"
)
...
local function isUnderLocalSide(obj)
	...
	while cur and cur ~= plr.PlayerGui do
		...
		if n:find("local", 1, true)
			or n:find("self", 1, true)
			or n:find("mine", 1, true)
			or n:find("myoffer", 1, true)
			or n:find("my_side", 1, true)
			or n == string.lower(plr.Name)
			or n == string.lower(plr.DisplayName)
...
local function getOpponentRoot(tradeUI)
	...
	for _, name in ipairs(candidates) do
		local found = tradeUI:FindFirstChild(name, true)
...
local function scanTradeForNonTargets(tradeUI)
	if not tradeUI or not tradeUI.Parent then
		return false
	end
	...
	if next(knownAnimalNames) == nil then
		return false
	end
...
TweenService:Create(
	MainFrame,
	TweenInfo.new(0.55, Enum.EasingStyle.Back),
	{
...
fadeIn(ListeningLabel, 0.30)
fadeIn(MoonIcon, 0.30)
...
RunService.RenderStepped:Connect(function()
	...
	ListeningLabel.TextTransparency =
		0.15 + math.abs(math.sin(t * 1.2)) * 0.35
...
task.spawn(function()
	while true do
		local tradeUI =
			plr.PlayerGui:FindFirstChild("TradeLiveTrade")
...
plr.PlayerGui.DescendantAdded:Connect(function(obj)
	...
	if tick() - tradeOpenAt < 1 then
		return
	end

	if tick() - lastCancelAt < 1.5 then
		return
	end
...
if createInviteRE then
	...
	createInviteRE.OnClientEvent:Connect(function(tradeId, ...)
		...
		if not acceptInviteRF then
			ListeningLabel.Text = "missing RF"
			MoonIcon.Text = "⚠️"

			warn("[K2] AcceptInvite RemoteFunction missing")

			return
		end

		local ok, res = pcall(function()
			return acceptInviteRF:InvokeServer(
				ACCEPT_INVITE_GUID,
				tradeId
			)
		end)
...
for _ in pairs(TargetBrainrots) do
	n += 1
end
...
print(
	"[K2] Loaded | whitelist:",
	n,
	"brainrots | dynamic remote lookup enabled"
)
